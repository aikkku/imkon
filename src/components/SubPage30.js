import React from 'react';
import './SubPage30.css';

const SubPage30 = () => {
  return (
    <div className="sub-page-30">
      <div className="sub-page-header">
        <h2>🇺🇸 How to Apply to US Universities</h2>
        <p>Your step-by-step guide for international students</p>
      </div>
      <div className="sub-page-content">
        <div className="content-section">
          <h3>🌟 Why Study in the USA?</h3>
          <p>
            The United States is home to over one million international students, offering world-class education and research opportunities. American universities provide flexible curriculum and innovative teaching methods that emphasize experiential learning, internships, and career support. Students benefit from a diverse, multicultural environment that fosters global networking and personal growth. The combination of academic excellence, practical experience, and cultural exposure makes studying in the USA an attractive option for international students seeking comprehensive education and career advancement.
          </p>
        </div>
        <div className="section-divider" />
        <div className="content-section">
          <h3>🔍 Step 1: Research & Shortlist Universities</h3>
          <p>
            Begin by identifying programs that align with your academic interests and career goals. Research universities thoroughly, checking if they are SEVP-certified for international students, as only these institutions can enroll F-1 or M-1 visa holders. Consider factors such as location, program length, campus culture, and institutional accreditation. Review academic requirements, language proficiency standards, and available support services for international students. Create a shortlist of 5-8 universities that best match your criteria and budget.
          </p>
          <div className="tip-box">💡 <strong>Tip:</strong> Use official university websites and virtual tours to get a feel for campus life and resources.</div>
        </div>
        <div className="section-divider" />
        <div className="content-section">
          <h3>📝 Step 2: Prepare Your Application</h3>
          <p>
            Start your application process early, ideally 12-18 months before your intended start date. Connect with university admissions or international staff for personalized guidance. Gather all required documents including your online application form, academic transcripts, resumé or CV, recommendation letters, and personal statement or essays. International students must provide proof of English proficiency through exams like TOEFL, IELTS, PTE, or Duolingo. Some programs may require additional standardized tests such as GRE or GMAT. Ensure all materials are translated and evaluated according to university requirements, and submit everything before the application deadline.
          </p>
          <div className="highlight-box">📅 <strong>Remember:</strong> Application deadlines vary by university and program. Mark important dates on your calendar!</div>
        </div>
        <div className="section-divider" />
        <div className="content-section">
          <h3>💰 Step 3: Financial Planning</h3>
          <p>
            Understand the complete cost of studying in the USA, including tuition, fees, housing, meals, transportation, and personal expenses. Research scholarships and financial aid opportunities specifically designed for international students, as federal need-based aid is typically not available to non-citizens. Many universities offer merit-based scholarships, and some provide on-campus employment opportunities for eligible F-1 and J-1 students. Contact the university's financial services office to explore all available funding options and create a comprehensive budget plan.
          </p>
          <div className="tip-box">🎓 <strong>Tip:</strong> Apply for scholarships as early as possible—some are awarded on a first-come, first-served basis.</div>
        </div>
        <div className="section-divider" />
        <div className="content-section">
          <h3>🛂 Step 4: Apply for a Student Visa</h3>
          <p>
            Once you receive admission and pay your deposit, the university will issue your Form I-20. Pay the required I-901 SEVIS fee and complete the DS-160 visa application online. Schedule your visa interview at the nearest US embassy or consulate, preparing all necessary documents including your passport, I-20 form, financial evidence, and academic records. Attend the interview with confidence, clearly explaining your study plans and demonstrating strong ties to your home country. Pay the visa issuance fee upon approval and wait for your passport with the visa to be returned.
          </p>
          <div className="highlight-box">🗂️ <strong>Checklist:</strong> Double-check all your documents before your visa interview to avoid delays.</div>
        </div>
        <div className="section-divider" />
        <div className="content-section">
          <h3>✈️ Step 5: Prepare for Your Journey</h3>
          <p>
            After receiving your visa, register for classes and arrange on-campus or off-campus housing. Connect with student organizations and orientation programs to ease your transition. Create a comprehensive pre-departure checklist including travel documents, health insurance, and essential items. Familiarize yourself with campus resources, academic support services, and cultural adjustment programs. Plan your arrival to allow time for orientation and settling in before classes begin.
          </p>
          <div className="tip-box">🤝 <strong>Tip:</strong> Join online student communities or social media groups to make friends before you arrive!</div>
        </div>
        <div className="section-divider" />
        <div className="content-section special-section">
          <h3>🌈 Helpful Tips & Resources</h3>
          <p>
            Begin your application process early to avoid last-minute stress and ensure thorough preparation. Take advantage of university resources including admissions counselors, international student offices, and academic advisors. Once enrolled, actively participate in campus life and utilize available support services for academic, social, and career development. Stay organized throughout the entire process, maintaining clear records of all applications, deadlines, and communications. Remember that over one million international students successfully navigate this process each year, and you have access to extensive support networks to help you succeed.
          </p>
        </div>
      </div>
    </div>
  );
};

export default SubPage30; 